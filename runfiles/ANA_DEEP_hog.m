function results = ANA_settings(seq, res_path, bSaveImage, parameters)
...
cnn_params.output_layer = [3];               % Which layers to use
cnn_params.downsample_factor = [2];           % How much to downsample each output layer
cnn_params.compressed_dim = [16];            % Compressed dimensionality of each output layer
...
% Rotate Augmentation
params.augment = 0; % first we disabled augmentation
params.augment_factor = 5;
params.augment_angle = [0,-20,20,-10,10]; % 30 [1,.4,.4] will succeed in Motor
params.augment_weights = [0.7,.1,.1,.1,.1]; % 15, [1,.5,.5] also succeed
params.augment_flags = [true,true,true]; % per feature flag, true means rotate, falsem eans no-rotate
