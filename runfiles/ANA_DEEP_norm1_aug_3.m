...
% Rotate Augmentation
params.augment = 1; % first we disabled augmentation
params.augment_factor = 5;
params.augment_angle = [0,-20,20,-10,10]; % 30 [1,.4,.4] will succeed in Motor
params.augment_weights = [0.9,.3,.3,.3,.3]; % 15, [1,.5,.5] also succeed
params.augment_flags = [true]; % per feature flag, true means rotate, falsem eans no-rotate
