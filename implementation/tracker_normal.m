...
assert(params.normal_rotation && params.augment);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Initialization
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while true
    ...
    if seq.frame == 1
        ...
        xlf_all = cell(0,1,num_feature_blocks);
        for i = 1:length(params.augment_angle)
            ang = params.augment_angle(i);
            
            % Extract rotated features
            xl = extract_rotated_features(im, sample_pos, currentScaleFactor, features, global_fparams, feature_extract_info, ang);
        ...
        for i = 1:size(xlf_all,3)
            sample_cell = xlf_all(:,1,i);
            xlf{i} = cat(3, sample_cell{:});
        end
...
% 			error('NO IMPLEMENTATION');
            ...
            for i = 1:length(params.augment_angle)
                ...
                xlf_all(end+1,:,:) = xlf_proj;
            end
            for i = 1:size(xlf_all,3)
                sample_cell = xlf_all(:,1,i);
                xlf_proj{i} = cat(3, sample_cell{:});
            end
        else
            error('NO IMPLEMENTATION');
            if params.debug
